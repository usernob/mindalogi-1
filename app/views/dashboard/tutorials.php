<div class="w-full">
        ...
        <?php foreach ($data["db"]["tutorial"]["free"] as $tutorial) : ?>
            <div class="rounded-md shadow-md flex flex-col items-center bg-black overflow-hidden">
                ...
                <div class="w-full flex items-center justify-between p-4">
                    ...
                    <a class="body-text text-primer4" href="<?= BASE_URL ?>/tutorial/details/<?= $tutorial["id_tutorial"] ?>">view tutorial</a>
                </div>
            </div>
        <?php endforeach ?>
    </div>
    ...
    <?php else : ?>
        <div>
            <p class="body-text text-slate-600">You need to be premium to see all tutorial</p>
        </div>
    <?php endif ?>
</div>